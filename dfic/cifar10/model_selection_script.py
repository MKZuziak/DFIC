...
def model_evaluation():
    ...
    train_set = train_set.map(lambda sample: {'img': convert_tensor(sample['img'])})
    train_set.set_format('pt', columns=['img'], output_all_columns=True)
    validation_set = validation_set.map(lambda sample: {'img': convert_tensor(sample['img'])})
    validation_set.set_format('pt', columns=['img'], output_all_columns=True)
    test_set = test_set.map(lambda sample: {'img': convert_tensor(sample['img'])})
    test_set.set_format('pt', columns=['img'], output_all_columns=True)
